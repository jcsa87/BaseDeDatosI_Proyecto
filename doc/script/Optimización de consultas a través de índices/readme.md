Prueba 1: Sin Índice
Plan de Ejecución: Examen de tabla
Costo del Operador: 100%
Lecturas Lógicas: 3248 
Tiempo CPU: 78 ms
Tiempo Transcurrido: 190 ms
Observación: Tuvo que leer la tabla entera (1,000,000 de filas) para encontrar 16505.
Prueba 2: Con Índice Agrupado (Simple)
Plan de Ejecución: Búsqueda en índice cluster
Costo del Operador: 100%
Lecturas Lógicas: 55 
Tiempo CPU: 0 ms 
Tiempo Transcurrido: 144 ms
Observación: Leyó solo las 16505 filas necesarias.
Prueba 3: Con Índice Agrupado COMPUESTO
(Sobre fecha_emision, id_cliente, id_factura)
Plan de Ejecución: Búsqueda en índice clúster
Costo del Operador: 100%(Esto es normal, significa que es el único paso)
Lecturas Lógicas: 55
Tiempo CPU: 0 
Tiempo Transcurrido: 148 ms 
Observación: Rendimiento casi idéntico a la Prueba 2. También fue directo a los datos.
...
Tabla HEAP (Prueba 1): Es una tabla que no tiene un índice agrupado. Los datos no tienen un orden físico específico; simplemente se almacenan donde haya espacio. Es el análogo a una pila de papeles desordenada.
Índice Agrupado (Clustered Index - Pruebas 2 y 3): Esta es la tabla misma, ordenada físicamente. El índice es la tabla. Solo puede existir un índice agrupado por tabla. Es el análogo a un diccionario, donde las palabras (los datos) ya están físicamente ordenadas.
...
Consulta de Prueba: Se ejecutó la misma consulta en todos los escenarios, buscando un rango de fechas.
SQL
SELECT id_factura, id_cliente, fecha_emision
FROM factura
WHERE fecha_emision BETWEEN '2024-01-01' AND '2024-01-31';
Estadisticas: Se registraron las siguientes estadísticas en cada prueba, limpiando el caché (DBCC DROPCLEANBUFFERS) antes de cada ejecución:
...
78 ms
190 ms
2. Índice Agrupado Simple
Clustered Index Seek
55
0 ms
144 ms
3. Índice Agrupado Compuesto
Clustered Index Seek
55
0 ms
148 ms
...
Conclusión: Para esta consulta específica, la ganancia de rendimiento ya se había obtenido en la Prueba 2, porque la clausula WHERE solo usaba la primera columna del índice fecha_emision. El rendimiento no mejoró, pero tampoco empeoró.
Conclusión Final
